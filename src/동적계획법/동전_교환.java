...
public class 동전_교환 {
	...
	public long getWays(int n, int[] c) {
		// TODO(human): 구현하세요.
		memo = new int[n + 1][c.length];
		for (int[] ints : memo) {
			Arrays.fill(ints, -1);
		}
		return getTotalCount(0, c, n, 0);
	}

	public int getTotalCount(int startIdx, int[] coins, int target, int accum) {
		if (accum == target) return 1;
		if (accum > target) return 0;
		if (memo[accum][startIdx] != -1) {
			return memo[accum][startIdx];
		}

		int totalCount = 0;
		for (int i = startIdx; i < coins.length; i++) {
			totalCount += getTotalCount(i, coins, target, accum + coins[i]);
		}
		memo[accum][startIdx] = totalCount;
		return totalCount;
	}
